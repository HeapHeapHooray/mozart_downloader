...
PAGE_URL="https://splice.com/instrument/download"
TARGET="${1:-.}"

echo "Fetching Splice download page..."
PAGE_HTML=$(curl -sL "$PAGE_URL" || true)

# Extract Windows installer URL (.exe) or fallback to stable release URL
INSTALLER_URL=$(echo "$PAGE_HTML" | grep -oE 'https://[^"]+\.exe' | head -n 1 || true)

if [ -z "$INSTALLER_URL" ]; then
    INSTALLER_URL="https://desktop.splice.com/conveyor/stable/splice.exe"
fi
...
echo "=========================================="
echo "Splice Desktop Windows Installer Downloader"
echo "=========================================="
